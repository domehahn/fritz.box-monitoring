...
def build_report(
    q: Scalar, qv: Vector, window: str = "7d", period: str = "Weekly"
) -> Tuple[str, str]:
    """Return (title, markdown_body).

    ``period`` ("Weekly" / "Monthly") only changes the wording; ``window`` is the
    PromQL range that actually decides how far back everything looks.
    """
    span = {"Weekly": "the week", "Monthly": "the month"}.get(period, "the period")
    L: List[str] = []
    ...
    L.append(f"**Internet reachable** — {_pct(internet)} of {span}")
    ...
    if talkers:
        L.append(f"**Top bandwidth (download, {span.split()[-1]}):**")
        for m, v in talkers:
            ip = m.get("ip", "?")
    ...
    title = f"📊 {period} network digest — health {_pct(score)}"
    return title, "\n".join(L)
